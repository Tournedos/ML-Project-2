...
class CandidatePointsDetect:
    ...
    def calc_lapmat_of_inner_part(self, distance_matrix):
        for i in range(1, distance_matrix.shape[0] - 1):
            for j in range(1, distance_matrix.shape[1] - 1):
                if distance_matrix[i, j] > 2:
                    self.laplacian_matrix[i, j] = (
                        distance_matrix[i - 1, j] + distance_matrix[i + 1, j] +
                        distance_matrix[i, j - 1] + distance_matrix[i, j + 1] -
                        4 * distance_matrix[i, j]
                    )
    ...
    def catch_candidate_by_lapmat(self, candidate_points: CandidatePoints):
        ...
        for i in range(1, self.distance_matrix_modification.shape[0] - 1):
            candidate_points.add_line()
            for j in range(1, self.distance_matrix_modification.shape[1] - 1):
    ...
    def contour_range_get(self, contours, contour_idx):
        ...
        self.worm_xy_range = [
            ...
            min(y_max + BW.SIDE_WIDTH, WORM.IMAGE_SIZE2)
        ]
    ...
    def detect_points(self, image, candidate_points, width, area):
        ...
        self.calc_lapmat_of_inner_part(self.distance_matrix)
        ...
        self.calc_lapmat_of_inner_part(self.distance_matrix_modification)
